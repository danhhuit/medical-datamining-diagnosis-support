...
def show_result(result: Dict[str, Any], lang: str = "vi", inputs: Dict[str, Any] = None) -> None:
    ...
    # ── Tóm tắt thông số bệnh nhân đã dùng để dự đoán ──
    if inputs:
        inputs_title = "Thông số hồ sơ đang đánh giá" if lang == "vi" else "Active patient profile parameters"
        age_label = "Tuổi" if lang == "vi" else "Age"
        sex_label = "Giới tính" if lang == "vi" else "Sex"
        gender_label = (t("sex_male", lang) if inputs.get("sex") == 1 else t("sex_female", lang)).lower()
        cp_val = inputs.get("cp", 0)
        cp_label = t(f"cp_{cp_val}", lang).split(" – ")[-1] if lang == "vi" else t(f"cp_{cp_val}", lang)
        cp_label = cp_label.lower()
        bp_label = "Huyết áp" if lang == "vi" else "Blood Press"
        chol_label = "Cholesterol" if lang == "vi" else "Cholesterol"
        hr_label = "Nhịp tim tối đa" if lang == "vi" else "Max Heart Rate"
        
        st.markdown(
            f"<div style='background-color: rgba(128, 128, 128, 0.08); padding: 12px; border-radius: 6px; margin: 10px 0px; border-left: 4px solid #1a3a5c; font-size: 13px;'>"
            f"<b>🔍 {inputs_title}:</b><br>"
            f"• {age_label}: <b>{inputs.get('age')}</b> | {sex_label}: <b>{gender_label}</b> | Đau ngực: <b>{cp_label}</b><br>"
            f"• {bp_label}: <b>{inputs.get('trestbps')} mmHg</b> | {chol_label}: <b>{inputs.get('chol')} mg/dl</b> | {hr_label}: <b>{inputs.get('thalach')} bpm</b>"
            f"</div>",
            unsafe_allow_html=True
        )

    # ── Biểu đồ phân phối xác suất bằng Plotly (tương tác) ──
    if probabilities and len(probabilities) == 2:
        ...
        fig = go.Figure(data=[
            go.Bar(
                ...
                marker_color=colors,
                width=0.4,
                text=[f"{y_values[0]:.1f}%", f"{y_values[1]:.1f}%"],
                textposition='auto',
                hovertemplate="<b>%{x}</b><br>Xác suất: %{y:.1f}%<extra></extra>" if lang == "vi" else "<b>%{x}</b><br>Probability: %{y:.1f}%<extra></extra>"
            )
        ])
        
        fig.update_layout(
            title={
                'text': f"<b>{t('result_chart_title', lang)}</b>",
                'y': 0.9,
                'x': 0.5,
                'xanchor': 'center',
                'yanchor': 'top',
            ...
            yaxis=dict(title=t("result_chart_y", lang), range=[0, 115]),
            height=300,
        ...
        if not api_key:
            ...
            try:
                ...
                if cache_key in st.session_state["gemini_advice_cache"]:
                    ...
                    # Cập nhật lời khuyên vào lịch sử (nếu có trong session state)
                    if "history" in st.session_state:
                        for entry in st.session_state["history"]:
                            entry_inputs_str = json.dumps(entry["inputs"], sort_keys=True) if entry["inputs"] else ""
                            if entry_inputs_str == inputs_str and entry["result"].get("prediction") == prediction:
                                entry["gemini_advice"] = advice_text
                                break
                else:
                    with st.spinner(
...
"""
                        ...
                        # Cập nhật lời khuyên vào lịch sử (để xem lại và xuất file)
                        if "history" in st.session_state:
                            for entry in st.session_state["history"]:
                                entry_inputs_str = json.dumps(entry["inputs"], sort_keys=True) if entry["inputs"] else ""
                                if entry_inputs_str == inputs_str and entry["result"].get("prediction") == prediction:
    ...
    # ── Nút tải kết quả (đầy đủ thông tin đầu vào + kết quả) ──
    download_row = {}
    if inputs:
        download_row.update({
            "age": inputs.get("age"),
    ...
    # Lấy thông tin gợi ý y khoa từ Gemini AI
    advice_text = ""
    if inputs:
        inputs_str = json.dumps(inputs, sort_keys=True) if inputs else ""
        cache_key = hashlib.md5(f"{inputs_str}_{prediction}_{lang}".encode("utf-8")).hexdigest()
    ...
    download_row.update({
        ...
        "probability_class_0": f"{probabilities[0]*100:.1f}%" if probabilities and len(probabilities) == 2 else "",
        "probability_class_1": f"{probabilities[1]*100:.1f}%" if probabilities and len(probabilities) == 2 else f"{prob*100:.1f}%",
